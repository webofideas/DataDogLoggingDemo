...
WORKDIR /src
COPY ["DataDogLoggingDemo/DataDogLoggingDemo.csproj", "DataDogLoggingDemo/"]
RUN dotnet restore "DataDogLoggingDemo/DataDogLoggingDemo.csproj"
COPY . .
WORKDIR "/src/DataDogLoggingDemo"
RUN dotnet build "DataDogLoggingDemo.csproj" -c Release -o /app/build
 
FROM build AS publish
RUN dotnet publish "DataDogLoggingDemo.csproj" -c Release -o /app/publish
...
ENV DD_SERVICE="datadogloggingdemo"
ENV DD_VERSION="1.0.0.0"
ENV DD_ENV="Development"

WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "DataDogLoggingDemo.dll"]